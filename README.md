# wip
$${"Liebe, \space so \space klingt \space Liebe}$$
$${Leiser \space als \space normal, \space doch \space viel \space lauter \space als das \space letzte \space Mal"}$$
 
 
 
 
 
$${Haii \space im \space jason, \space im \space a \space yumeshipper \space and \space kinnie!!}$$
$${check \space my \space pronouns \space page}$$

<p align="center">
<img src="https://media.tenor.com/neRZrojlfIcAAAAi/divider-aesthetic.gif" alt="a black and white image of a striped pattern on a white background"/>

<p align="center">


<p align="center">
<img src="https://media.tenor.com/neRZrojlfIcAAAAi/divider-aesthetic.gif" alt="a black and white image of a striped pattern on a white background"/>

<p align="center">
<img src="https://blinkies.cafe/b/display/0023-trans-pride.gif" alt="this user is trains blinkie"/>

$${DNI:}$$

$${MAGA}$$

$${(iwec) \space HASBINHOTEL \space FANS}$$

$${(iwec) \space HELAVABOSS \space FANS}$$

$${HOMOPHOBIC \space PEOPLE}$$

$${UNDER \space 13}$$

$${RACISTS}$$

$${JUST \space RUDE \space PEOPLE \space GO \space AWAY}$$

$${ANTI \space ABORTION}$$

<p align="center">
<img src="https://media.tenor.com/neRZrojlfIcAAAAi/divider-aesthetic.gif" alt="a black and white image of a striped pattern on a white background"/>

<p align="center">

 
<p align="center">
<img src="https://media.tenor.com/neRZrojlfIcAAAAi/divider-aesthetic.gif" alt="a black and white image of a striped pattern on a white background"/>



